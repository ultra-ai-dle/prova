...
let arr = [3, 1, 2]; __snap(1, () => ({arr}));
arr.sort((a, b) => a - b); __snap(2, () => ({arr}));
console.log(arr[0]); __snap(3, () => ({arr}));
```
...
function parseUserSymbols(code) {
  ...
  function collectPattern(node, set) {
    if (!node) return;
    if (node.type === "Identifier") { set.add(node.name); return; }
    if (node.type === "ArrayPattern") { node.elements.forEach(e => collectPattern(e, set)); return; }
    if (node.type === "ObjectPattern") { node.properties.forEach(p => collectPattern(p.value, set)); return; }
  }
...
function instrumentCode(code, symbols) {
  ...
  function visitStatements(body) {
    for (const stmt of body) {
      ...
      insertions.push({ offset: endOffset, text: `; __snap(${line}, () => (${capture}))` });
...
function _safeJs(value, depth = 0, safeL0 = 30, safeLN = 12) {
  ...
  if (Array.isArray(value)) {
    const sliced = value.slice(0, limit).map(v => _safeJs(v, depth + 1, safeL0, safeLN));
  ...
  if (value instanceof Map) {
    ...
    for (const [k, v] of value) {
      if (count++ >= limit) { out["..."] = `+${value.size - limit} items`; break; }
      out[String(_safeJs(k, depth + 1, safeL0, safeLN))] = _safeJs(v, depth + 1, safeL0, safeLN);
    }
    return out;
  }
  if (value instanceof Set) {
    return [...value].slice(0, limit).map(v => _safeJs(v, depth + 1, safeL0, safeLN));
  }
  if (typeof value === "object" && value !== null) {
    const out = {};
    let count = 0;
    for (const [k, v] of Object.entries(value)) {
      if (count++ >= limit) { out["..."] = `+${Object.keys(value).length - limit} items`; break; }
...
function executeInSandbox(instrumentedCode, stdin, limits) {
  ...
  // step 0: 초기 상태
  trace.push({ step: 0, line: 0, vars: {}, scope: { func: "<global>", depth: 1 },
                parent_frames: [], stdout: [], runtimeError: null });
  step = 1;
  ...
  function __snap(line, captureThunk) {
    ...
    let rawVars = {};
    try { rawVars = captureThunk(); } catch {}
    const vars = {};
    ...
    trace.push({ step, line, vars,
                 scope: { func: "<global>", depth: 1 },
                 parent_frames: [], stdout: [...stdoutLines], runtimeError: null });
    step++;
  }
  ...
  const fakeConsole = {
    ...
    error: (...args) => stdoutLines.push("[error] " + args.map(String).join(" ")),
  };
  ...
  try {
    ...
    // 최종 상태 추가
    const lastVars = trace.length > 0 ? { ...trace[trace.length - 1].vars } : {};
    trace.push({ step, line: trace[trace.length - 1]?.line ?? 0, vars: lastVars,
                 scope: { func: "<global>", depth: 1 }, parent_frames: [],
                 stdout: [...stdoutLines], runtimeError: null });
    if (truncated) {
      const last = trace[trace.length - 1];
      last.stdout = [...stdoutLines, "[warn] 추적 단계 한도를 초과해 일부 단계가 생략되었습니다."];
    }
  } catch (err) {
    // SyntaxError: Acorn이 잡지 못한 케이스 또는 런타임 에러
    const lineNo = extractErrorLine(err, instrumentedCode) ?? 0;
    trace.push({ step, line: lineNo, vars: {},
                 scope: { func: "<global>", depth: 1 }, parent_frames: [],
                 stdout: [...stdoutLines],
                 runtimeError: { type: err.constructor.name, message: err.message, line: lineNo } });
  }
  return trace;
}
...
self.onmessage = (event) => {
  ...
  if (!code.trim()) {
    self.postMessage({ type: "invalid_input", message: "코드를 입력한 후 디버깅을 시작하세요." });
    return;
  }
  ...
  try {
    ...
    rawTrace = [{
      step: 0, line: 0, vars: {}, scope: { func: "<global>", depth: 1 },
      parent_frames: [], stdout: [],
  ...
  self.postMessage({ type: "done", rawTrace, branchLines: { loop: [], branch: [] }, varTypes });
};
...
```ts
function applyJsQueueHints(meta: AnalyzeMetadata, code: string, varTypes: Record<string, string>): AnalyzeMetadata {
  ...
  const queueVarMatch = code.match(/\b(let|const|var)\s+([A-Za-z_$][A-Za-z0-9_$]*)\s*=\s*\[/g);
  // ... 간단한 이름 기반 감지
  return { ...meta, tags: normalizeAndDedupeTags([...(meta.tags ?? []), "queue", "bfs"], 10) };
}
```
...
2. 브라우저 콘솔에서 직접 Worker 테스트:
   ...
   w.postMessage({ code: "let arr = [3,1,2]; arr.sort((a,b)=>a-b);", stdin: "" });
...
> JavaScript는 stdin 없이 실행 가능하므로 Input 필드를 비워도 됩니다.
> stdin을 사용하는 경우에만 예시 입력을 작성하세요.

---

### [1] 버블 정렬 — LINEAR + index pivot
...
**기대 varTypes:** `{ arr: "list", i: "int", j: "int", tmp: "int" }`  
**기대 strategy:** `LINEAR`
...
**기대 varTypes:** `{ graph: "dict", queue: "list", node: "int" }`  
**기대 strategy:** `GRAPH` 또는 `GRID_LINEAR`
...
**기대 varTypes:** `{ n: "int", dp: "list", i: "int" }`  
**기대 strategy:** `LINEAR`

---

### [4] 이진 탐색 — LINEAR + 투포인터 pivot
...
**기대 varTypes:** `{ arr: "list", target: "int", lo: "int", hi: "int", mid: "int" }`  
**기대 strategy:** `LINEAR`

---

### [5] stdin 사용 예시 — readline()

JS에서 stdin을 읽을 때는 `readline()` 함수를 사용합니다.
...
```js
// 기존: 문자열 오프셋 기반 삽입 (깨짐)
result = result.slice(0, stmt.end) + "; __snap(line, () => ({...}))" + result.slice(stmt.end);
...
function visitStatements(body) {
  const newBody = [];
  for (const stmt of body) {
    const isExit = ["ReturnStatement","ThrowStatement","BreakStatement","ContinueStatement"]
      .includes(stmt.type);
...
function highlightJsLine(line: string): Array<{ text: string; className: string }> {
  ...
  while (match) {
    ...
    const token = match[0];
    if (token.startsWith("//"))       tokens.push({ text: token, className: "text-[#8b949e] italic" });
    else if (/^["`']/.test(token))    tokens.push({ text: token, className: "text-[#a5d6ff]" });
    else if (/^\d/.test(token))       tokens.push({ text: token, className: "text-[#79c0ff]" });
    else if (JS_KEYWORDS.has(token))  tokens.push({ text: token, className: "text-[#ff7b72]" });
    else                              tokens.push({ text: token, className: "text-[#d2a8ff]" });
    lastIndex = match.index + token.length;
    match = pattern.exec(line);
  }
  if (lastIndex < line.length) tokens.push({ text: line.slice(lastIndex), className: "text-[#c9d1d9]" });
  if (tokens.length === 0) tokens.push({ text: " ", className: "text-[#c9d1d9]" });
  return tokens;
}
```
...
```ts
const langLabel = language === "javascript" ? "JavaScript" : "Python";
const langSpecificHints = language === "javascript" ? [
