...
> Frogger에 JavaScript(ES2022, 동기 코드 한정) 지원을 추가하기 위한 실행 가능한 구현 설계입니다.
...
`FroggerRuntime`이 언어를 받아 올바른 Worker 파일을 선택하도록 변경합니다.
...
#### (a) FroggerRuntime 생성 시 language 전달

```ts
// 현재 (~line 280)
const [language, setLanguage] = useState('python');

// FroggerRuntime 생성 부분에 language 전달
// (FroggerRuntime init/재생성 로직에서)
runtimeRef.current = new FroggerRuntime(callbacks, language);
```
...
useEffect(() => {
  ...
  // 새 언어로 초기화
  const rt = new FroggerRuntime(runtimeCallbacks, language);
  rt.init();
...
// JS Worker가 출력하는 내부 변수(__snap, __frogger* 등)를 필터
...
5. `page.tsx` 수정: language state → FroggerRuntime 재생성 effect
...
| `src/types/frogger.ts`              | `SupportedLanguage` 타입 추가 (+2줄)             | 낮음   |
...
> `FroggerRuntime` 생성자 시그니처 변경(STEP 2)이 먼저 완료되어야 함
...
### STEP 5 — (선택) `src/types/frogger.ts` 타입 추가
...
`language` state와 `FroggerRuntime` 생성자 파라미터 타입에 `string` 대신 `SupportedLanguage` 적용.
...
[ ] STEP 5  frogger.ts  — SupportedLanguage 타입 (선택)
