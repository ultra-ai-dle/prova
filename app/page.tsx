...
import { FroggerRuntime } from '@/features/execution/runtime';
import { AnalyzeMetadata, RawTraceStep } from '@/types/frogger';
import { useFroggerStore } from '@/store/useFroggerStore';
...
const LAST_EXECUTED_CODE_KEY = 'frogger:lastExecutedCode';
const LAST_EXECUTED_STDIN_KEY = 'frogger:lastExecutedStdin';
const LAST_SELECTED_LANGUAGE_KEY = 'frogger:lastSelectedLanguage';
...
export default function Page() {
  ...
  const runtimeRef = useRef<FroggerRuntime | null>(null);
  ...
  useEffect(() => {
    ...
    const runtime = new FroggerRuntime(
      {
  ...
  const headerBadge = useMemo(() => {
    ...
    if (isFallback)
      return {
        text: '○ 알고리즘 감지 실패',
        style: 'border-frogger-line text-frogger-muted',
      };
    if (metadata?.display_name)
      return { text: '', style: 'border-frogger-line text-frogger-muted' };
    return {
  ...
  return (
    <div className='h-screen flex flex-col bg-frogger-bg text-[#e6edf3] overflow-hidden'>
      {/* ── Running progress bar ────────────────────────────── */}
      <div
        className={`h-[2px] shrink-0 transition-opacity duration-300 ${isRunning ? 'opacity-100 animate-pulse bg-gradient-to-r from-[#58a6ff] via-frogger-green to-[#58a6ff]' : 'opacity-0'}`}
      />
      ...
      {/* ── Header ──────────────────────────────────────────── */}
      <header className='shrink-0 h-11 bg-[#161b22] border-b border-frogger-line flex items-center px-3 gap-4'>
        {/* Logo */}
        <div className='font-bold text-[15px] tracking-tight shrink-0'>
          Pro<span className='text-frogger-green'>va</span>
        </div>
        ...
        <button
          className='w-7 h-7 flex items-center justify-center rounded text-frogger-muted hover:text-[#c9d1d9] hover:bg-[#21262d] transition-colors shrink-0'
          aria-label='설정'
          title='설정'
        >
          <IconSettings />
        ...
        <div ref={splitRootRef} className='flex-1 flex min-h-0 min-w-0'>
          ...
          >
            ...
            <div
              className={`shrink-0 h-9 flex items-center justify-between px-3 border-b transition-colors ${
                isDebugMode
                  ...
                  : 'border-frogger-line bg-[#0f141a]'
              }`}
            >
              <div className='flex items-center gap-2 min-w-0'>
                <span className='text-[10px] text-frogger-muted uppercase tracking-widest font-medium truncate'>
                ...
                {isVisualizing && (
                  <span
                    className={`shrink-0 text-[10px] px-2 py-[2px] rounded-full border font-medium ${
                      isError
                        ? 'border-frogger-red/40 bg-[#2d1112]/60 text-frogger-red'
                        : 'border-frogger-green/40 bg-[#1a4731]/60 text-frogger-green'
                    }`}
                  >
                    {isError ? 'error' : 'ready'}
                  </span>
              ...
              <div className='flex items-center gap-2 shrink-0'>
                <select
                  className='h-7 rounded border border-frogger-line bg-[#161b22] text-[11px] text-[#c9d1d9] px-2 focus:outline-none'
                  value={language}
                ...
                <button
                  className='h-7 px-2 rounded border border-frogger-line bg-[#21262d] text-[10px] font-mono text-[#c9d1d9] hover:border-[#58a6ff]/40 transition-colors'
            ...
            {displayTags.length > 0 && (
              <div className='shrink-0 px-3 py-2 border-b border-frogger-line bg-[#0d1117]'>
                <div className='flex items-center gap-2 flex-nowrap overflow-x-auto frogger-scrollbar whitespace-nowrap'>
                  {displayTags.map((tag) => (
                    <span
                      key={tag}
            ...
            {/* Code lines */}
            <div
              className={`flex-1 overflow-hidden relative transition-colors ${isDebugMode ? 'bg-[#0c1016]' : 'bg-frogger-bg'}`}
            >
              ...
              <div className='absolute top-2 right-2 z-10 flex items-center gap-1'>
                ...
                <button
                  className={`h-6 w-6 flex items-center justify-center rounded border transition-colors ${
                    wordWrap
                      ...
                      : 'border-frogger-line bg-frogger-panel/80 text-frogger-muted hover:text-[#c9d1d9] hover:border-[#58a6ff]/40'
                  }`}
                ...
                <button
                  className={`h-6 w-6 flex items-center justify-center rounded border transition-colors ${
                    copied
                      ? 'border-frogger-green/50 bg-[#1a4731]/60 text-frogger-green'
                      : 'border-frogger-line bg-frogger-panel/80 text-frogger-muted hover:text-[#c9d1d9] hover:border-[#58a6ff]/40'
                  }`}
                  onClick={() => {
              ...
              {!isDebugMode ? (
                <div className='relative w-full h-full overflow-hidden'>
                  <div
                    ...
                    className={`absolute inset-0 frogger-scrollbar p-3 box-border font-mono text-[12px] leading-5 pointer-events-none ${wordWrap ? 'overflow-y-auto overflow-x-hidden' : 'overflow-auto'}`}
                    style={{ tabSize }}
                    aria-hidden='true'
                  >
                    {code.length === 0 ? (
                      <div className='flex border-l-2 border-[#58a6ff]/30'>
                        ...
                        <span
                          className={`pl-2 text-frogger-muted ${wordWrap ? 'whitespace-pre-wrap break-all' : 'whitespace-pre'}`}
                        >
                          {language === 'javascript'
                  ...
                  <textarea
                    ...
                    className={`absolute inset-0 w-full h-full resize-none frogger-scrollbar bg-transparent text-transparent caret-[#c9d1d9] font-mono text-[12px] leading-5 pl-14 pr-3 py-3 box-border outline-none ${wordWrap ? 'overflow-y-auto overflow-x-hidden whitespace-pre-wrap break-all' : 'overflow-auto whitespace-pre'}`}
                    style={{ tabSize }}
                    spellCheck={false}
                  />
                </div>
                ...
                <div
                  className={`box-border h-full p-3 frogger-scrollbar ${wordWrap ? 'overflow-y-auto overflow-x-hidden' : 'overflow-auto'}`}
                >
                  {code.split('\n').map((line, index) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex font-mono text-[12px] leading-5 transition-colors ${
                          error
                            ? 'bg-[#3d0b0b] border-l-2 border-frogger-red'
                            : active
            ...
            {/* Language badge */}
            <div className='shrink-0 px-3 py-2 border-t border-frogger-line bg-[#0f141a]'>
              <span className='text-[10px] text-frogger-muted font-mono'>
                {language === 'javascript'
          ...
          {/* Joint: left-center */}
          <div
            className='w-1 shrink-0 bg-frogger-line/70 hover:bg-[#58a6ff]/80 cursor-col-resize transition-colors'
            onMouseDown={() => {
          ...
          >
            <div className='shrink-0 min-h-9 flex items-center justify-between gap-2 px-3 py-1.5 border-b border-frogger-line bg-[#0f141a]'>
              <div className='flex items-center gap-2 min-w-0 flex-1'>
                <span className='text-[10px] text-frogger-muted uppercase tracking-widest font-medium shrink-0'>
                  {isFallback
                    ? 'Data Exploration'
                    : isAnalyzingCode
                      ? 'Analyzing...'
              ...
              {isVisualizing && (
                <div className='flex items-center gap-1 text-frogger-muted'>
                  {shouldShowBitToggle && (
                    <button
                      className={`h-6 px-2 rounded border text-[10px] font-mono transition-colors ${
                        bitmaskMode
                          ? 'border-[#58a6ff]/55 bg-[#15304e] text-[#9ac7ff]'
                          : 'border-frogger-line bg-[#161b22] text-frogger-muted hover:text-[#c9d1d9]'
                      }`}
              ...
              {isAnalyzingCode ? (
                <div className='h-full w-full grid place-items-center'>
                  <div className='text-center'>
                    ...
                    <p className='mt-2 text-xs text-frogger-muted'>
                      AI 응답을 기다리는 중입니다.
                    </p>
                  </div>
                </div>
          ...
          >
            {/* Debug controls */}
            <div className='shrink-0 border-b border-frogger-line bg-[#0f141a] px-3 py-2 space-y-2'>
              <div className='flex items-center justify-between'>
                <span className='text-[10px] text-frogger-muted uppercase tracking-widest font-medium'>
                  Debug Controls
                </span>
                <span className='text-[10px] text-frogger-muted font-mono'>
              ...
              <div className='flex items-center gap-2'>
                <button
                  className='h-7 px-2 flex items-center justify-center rounded border border-frogger-line bg-frogger-panel text-frogger-muted hover:text-white disabled:opacity-30 text-[10px] font-mono'
                ...
                <button
                  className={`h-7 px-2 flex items-center justify-center rounded border transition-colors text-[10px] font-mono ${
                    ...
                    playback.currentStep >= mergedTrace.length - 1
                      ? 'border-frogger-line bg-[#161b22] text-frogger-muted opacity-30 cursor-not-allowed'
                      : 'border-frogger-green/45 bg-[#12301f] text-frogger-green hover:bg-[#184329] hover:text-[#7ee787]'
                  }`}
                  onClick={() => setCurrentStep(playback.currentStep + 1)}
                  disabled={
                    isRunning ||
                ...
                <div className='ml-auto flex items-center gap-1'>
                  <span className='text-[10px] text-frogger-muted'>Speed</span>
                  <select
                    className='h-7 rounded border border-frogger-line bg-[#161b22] text-[10px] text-[#c9d1d9] px-1 focus:outline-none disabled:opacity-40'
            ...
            <div className='flex-1 min-h-0 flex flex-col'>
              ...
              >
                <div className='shrink-0 h-9 flex items-center justify-between px-3 border-b border-frogger-line bg-[#0f141a]'>
                  <span className='text-[10px] text-frogger-muted uppercase tracking-widest font-medium'>
                    Variable Monitor
                  </span>
                  <div className='flex items-center gap-1 text-frogger-muted'>
                ...
                <div className='shrink-0 px-3 py-[6px] bg-[#161b22] border-b border-frogger-line'>
                  <span className='text-[10px] text-frogger-muted font-mono'>
                    {currentStep
                ...
                <div className='flex-1 overflow-auto min-h-0 bg-[#0d1117]'>
                  {!currentStep && (
                    <div className='p-4 text-xs text-frogger-muted italic'>
                      실행 후 변수가 표시됩니다.
                    </div>
                  )}
                  {currentStep &&
                    Object.entries(currentStep.vars).map(([key, value]) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`font-mono truncate ${isKey ? 'text-frogger-green font-semibold' : 'text-[#8b949e]'}`}
                          >
                            {key}
                            {changed && (
              ...
              <div
                className='h-1 shrink-0 bg-frogger-line/70 hover:bg-[#58a6ff]/80 cursor-row-resize transition-colors'
                onMouseDown={() => {
                  dragTypeRef.current = 'var-input';
              ...
              >
                <div className='shrink-0 h-9 flex items-center justify-between px-3 border-y border-frogger-line bg-[#0f141a]'>
                  <span className='text-[10px] text-frogger-muted uppercase tracking-widest font-medium'>
                    Input
                  </span>
                  <span className='text-[10px] text-frogger-muted font-mono'>
                ...
                <div className='flex-1 min-h-0 bg-[#0d1117] p-3'>
                  <div className='h-full rounded-md border border-[#30363d] bg-[#161b22] p-3 flex flex-col gap-3'>
                    <textarea
                      className='flex-1 min-h-0 rounded border border-frogger-line bg-[#0d1117] text-xs font-mono p-2 resize-none placeholder:text-frogger-muted focus:outline-none focus:border-[#58a6ff]/60 transition-colors disabled:opacity-40'
                    ...
                    <button
                      className={`shrink-0 h-9 rounded text-xs font-bold tracking-wide transition-colors ${
                        ...
                        !isStdinEmpty
                          ? mergedTrace.length > 0
                            ? 'bg-[#21262d] border border-frogger-line text-[#c9d1d9] hover:bg-[#262c36]'
                            : 'bg-frogger-green text-black hover:bg-[#4ac763]'
                          : pyodideStatus === 'error'
                            ? 'bg-[#2d1112] border border-frogger-red text-[#f8b4b4]'
                            : 'bg-[#21262d] border border-frogger-line text-frogger-muted cursor-not-allowed'
                      }`}
                      disabled={
              ...
              >
                <div className='shrink-0 h-9 flex items-center justify-between px-3 border-y border-frogger-line bg-[#0f141a]'>
                  ...
                  <span
                    className={`text-[10px] font-mono ${isError ? 'text-frogger-red' : 'text-frogger-muted'}`}
                  >
                ...
                >
                  <div
                    className={`h-full rounded-md border px-3 py-2 overflow-auto ${
                      isError
                        ? 'border-frogger-red/40 bg-[#12090b] text-[#ffc1c1]'
                        : 'border-[#30363d] bg-[#0b1119] text-[#c9d1d9]'
                    }`}
                  >
                    {consoleLines.length === 0 ? (
                      <p className='text-frogger-muted'> </p>
                    ) : (
        ...
        {toasts.map((toast) => (
          <div
            ...
            className={`flex items-start gap-2 rounded-md border px-3 py-2 text-xs shadow-lg ${
              toast.kind === 'ok'
                ? 'bg-[#0d1f17] border-frogger-green text-frogger-green'
