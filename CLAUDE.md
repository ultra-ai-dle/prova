...
- **Execution**: Pyodide (Python WASM), Acorn (JS AST 계측), Web Worker, Java 원격 실행 서버
...
```
app/
├── api/analyze/     # AI: 알고리즘 분류 + 전략 결정
├── api/java/execute/ # Java 원격 실행 프록시
├── page.tsx         # 메인 페이지
src/
...
1. **TRACE** — Worker(Pyodide/JS) 또는 원격 JVM(Java)이 코드 실행 + trace 수집 → `{rawTrace, branchLines, varTypes}`
2. **AI** — `/api/analyze`(알고리즘 분류·전략·변수 매핑·enrichment)
...
- `README.md`를 자동 수정하지 않는다 — 수동 관리 대상
