...
import type { LinearPivotSpec } from '@/types/frogger';
...
function findFirstValueIndexInArray(
  arr: unknown[],
  needle: unknown,
  slotCount: number,
): number {
...
export function pointersAtIndexFromSpecs(
  ...
  linearPivots.forEach((spec, orderIdx) => {
    const applies =
      (typeof spec.indexes_1d_var === 'string' &&
        spec.indexes_1d_var === arrKey) ||
    ...
    const ringClass =
      RING_CLASS_BY_SPEC_ORDER[orderIdx % RING_CLASS_BY_SPEC_ORDER.length];
    const badge =
      spec.badge && spec.badge.length > 0
        ? spec.badge
        : defaultBadgeForVar(spec.var_name);
