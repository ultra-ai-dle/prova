      ...
      <div className="flex items-stretch overflow-auto">
        ...
        <div className="flex flex-col justify-between">
          ...
          <div className="flex items-center gap-0">
            {arr.length === 0 ? (
              <div className="h-10 px-6 border-y border-[#2d4f79] bg-[#0a1520] text-[11px] text-prova-muted font-mono flex items-center">
                empty
              </div>
            ) : (
              arr.map((v, i) => {
      ...
      {arr.length > 0 && (
        <div className="flex pl-[28px] gap-0">
          {arr.map((_, i) => (
            <div
              key={i}
              className="min-w-[38px] text-center text-[9px] text-prova-muted font-mono"
            >
              {i}
            </div>
          ))}
  ...
  return (
    <div className="py-2 px-1 inline-flex flex-col items-start gap-0">
      ...
      <div className="flex flex-col-reverse items-stretch w-full border border-[#2d4f79] rounded overflow-hidden">
        {arr.length === 0 ? (
          <div className="h-10 flex items-center justify-center text-[11px] text-prova-muted font-mono">
            empty
          </div>
        ) : (
          arr.map((v, i) => {
...
function VisitedView({ arr }: { arr: unknown[] }) {
  if (arr.length === 0) {
    return (
      <div className="text-[11px] text-prova-muted font-mono py-1">
        (비어있음)
      </div>
    );
  }
...
export function GraphPanel({
  ...
  if (!parsed || !step) {
    return (
      <div className="h-full grid place-items-center text-sm text-prova-muted">
        실행 후 그래프가 표시됩니다.
      </div>
    );
  }
        ...
        <div className="space-y-2">
          {parsed.structures.map((structure) => {
            ...
            return (
              <div
                key={structure.key}
                className="rounded border border-prova-line bg-[#0f141a] p-2"
              >
                        ...
                        >
                          ...
                          {Array.from({ length: maxCols }, (_, c) => (
                            <div
                              ...
                              className="text-[10px] text-prova-muted text-center font-mono"
                            >
                              x{c}
                            </div>
                          ))}
                          {grid.map((row, r) => (
                            <div
                              ...
                              className="contents"
                            >
                              <div className="text-[10px] text-prova-muted text-right pr-1 font-mono self-center">
                                y{r}
                              </div>
                              {Array.from({ length: maxCols }, (_, c) => (
                                <div
                        ...
                        <div className="flex items-start gap-1">
                          {Array.from({ length: slotLen }, (_, i) => {
                            ...
                            return (
                              ...
                              >
                                <div className="text-[10px] text-prova-muted font-mono tabular-nums">
                                  x{i}
                                </div>
                                <div
