...
function isScalar2DArray(value: unknown): value is unknown[][] {
  return (
    is2DArray(value) &&
    (value as unknown[][]).every((row) => row.every((cell) => isScalar(cell)))
  );
}
  ...
  return (
    ...
    (value as unknown[][][]).every(
      (plane) =>
        Array.isArray(plane) &&
        plane.every(
          (row) => Array.isArray(row) && row.every((cell) => isScalar(cell)),
        ),
    )
  );
}
...
function getFirst3DVar(step: MergedTraceStep) {
  ...
  const dpFirst = entries.find(
    ([name, value]) => /dp/i.test(name) && isScalar3DArray(value),
  );
...
function getFirst2DVar(step: MergedTraceStep) {
  ...
  if (scalarEntry)
    return { name: scalarEntry[0], value: scalarEntry[1] as unknown[][] };
  const entry = entries.find(([, value]) => is2DArray(value));
  return entry ? { name: entry[0], value: entry[1] as unknown[][] } : null;
}

function getFirstLinearVar(step: MergedTraceStep) {
  const entry = Object.entries(step.vars).find(
...
function list1DArrayKeys(step: MergedTraceStep): string[] {
  return Object.entries(step.vars)
    .filter(
      ([, value]) =>
        Array.isArray(value) &&
        (value.length === 0 || !Array.isArray((value as unknown[])[0])),
    )
    .map(([k]) => k);
}
...
function resolveLinearVar(step: MergedTraceStep, preferredName?: string) {
  if (preferredName) {
    ...
    if (
      Array.isArray(v) &&
      (v.length === 0 || !Array.isArray((v as unknown[])[0]))
    ) {
...
function toCells(
  step: MergedTraceStep,
  grid: unknown[][],
  previousGrid?: unknown[][] | null,
) {
  return grid.flatMap((row, y) =>
    row.map((value, x) => {
      const isCurrent =
        typeof step.vars.r === 'number' && typeof step.vars.c === 'number'
          ? step.vars.r === y && step.vars.c === x
          : false;
      const prevValue = previousGrid?.[y]?.[x];
      const changed = previousGrid
...
export function GridLinearPanel({
    ...
    return (
      <div className='h-full flex flex-col items-center justify-center text-center px-8 gap-4'>
        ...
        <div>
          ...
          <p className='text-xs text-frogger-muted leading-relaxed'>
    ...
    return (
      <div className='h-full overflow-auto p-4 text-xs'>
        <div className='flex items-center gap-2 mb-3'>
          <span className='text-[10px] text-frogger-muted uppercase tracking-widest'>
            Variable State
          </span>
        ...
        <div className='space-y-[2px]'>
          {vars.map(([key, value]) => {
            ...
            return (
              ...
              >
                <span
                  className={`font-mono truncate ${changed ? 'text-[#e3b341]' : 'text-frogger-muted'}`}
                >
                  {key}
                </span>
  ...
  const cells = shouldRenderGrid
    ? toCells(
        step,
        gridVar.value,
        previousGridVar && previousGridVar.name === gridVar?.name
          ? previousGridVar.value
          : null,
      )
    : [];
      ...
      <div className='h-full overflow-auto p-4 text-xs'>
        ...
        <div className='space-y-[2px]'>
          {vars.map(([key, value]) => (
            ...
            >
              <span className='font-mono truncate text-frogger-muted'>
                {key}
              </span>
              <span className='font-mono text-[#c9d1d9] truncate'>
                {JSON.stringify(value)}
    ...
    <div className='h-full flex flex-col'>
      ...
      {shouldRender3D && grid3DVar && (
        <ThreeDVolumePanel
          ...
          prevVolume={
            prev3DVar && prev3DVar.name === grid3DVar.name
              ? prev3DVar.value
              : null
          }
        ...
        <div className='flex-1 flex items-center justify-center p-4'>
          <div className='inline-grid gap-[5px]'>
            <div
              ...
              style={{
                gridTemplateColumns: `26px repeat(${(gridVar?.value?.[0] ?? []).length || 1}, 40px)`,
              }}
            >
              <div />
              {Array.from(
                { length: (gridVar?.value?.[0] ?? []).length || 0 },
                (_, c) => (
                  <div
                    key={`col-${c}`}
                        ...
                        <div
                          ...
                          className={`w-10 h-10 rounded-md border grid place-items-center text-sm font-bold transition-all duration-200 ${
                            hasError && cell.isCurrent
                              ? 'border-frogger-red bg-[#3d0b0b] text-frogger-red'
                              : cell.isCurrent
                                ? 'border-frogger-green bg-[#0d4429] text-frogger-green'
                                : cell.changed
                                  ? 'border-[#388bfd] bg-[#1f3555]/70 text-[#9ac7ff]'
                                  : 'border-[#21262d] bg-[#161b22] text-[#c9d1d9]'
                          }`}
                        >
                          {cell.isCurrent && !hasError
                            ? '→'
                            : formatCellValue(
                                cell.value,
                                bitmaskMode,
      ...
      {hasError && step.runtimeError && (
        <div className='shrink-0 mx-4 mb-2 px-3 py-2 rounded border border-frogger-red/40 bg-[#2d1112]/60 text-xs flex items-center gap-2'>
          <span className='text-frogger-red font-bold'>✕</span>
      ...
      {!hasError && linearVar && (
        <div className='shrink-0 border-t border-frogger-line px-4 py-3 space-y-2'>
          <div className='flex items-center gap-2 flex-wrap'>
            <span className='text-[10px] text-frogger-muted uppercase tracking-widest shrink-0'>
              {linearVar.name}
            </span>
            <span className='text-[10px] text-frogger-muted font-mono shrink-0'>
          ...
          <div className='flex items-start gap-1 overflow-x-auto frogger-scrollbar pb-1'>
            {queue.length === 0 ? (
              <span className='text-[11px] text-frogger-muted italic'>
                배열이 비어 있습니다.
              </span>
            ) : (
              queue.map((item, i) => {
                ...
                return (
                  <div
                    key={`lin-${i}`}
                    className='flex flex-col items-center gap-0.5 shrink-0 min-w-[34px]'
                  >
