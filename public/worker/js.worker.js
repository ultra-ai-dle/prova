...
function _safeJs(value, depth, safeL0, safeLN) {
  ...
  if (Array.isArray(value)) {
    ...
    for (let idx = 0; idx < len; idx++) {
      ...
      sliced.push(
        idx in value ? _safeJs(value[idx], depth + 1, safeL0, safeLN) : null,
      );
    }
    if (value.length > limit)
      sliced.push('...(+' + (value.length - limit) + ')');
    return sliced;
  }
  ...
  function walk(node) {
    ...
    if (
      ...
      if (node.left.type === 'VariableDeclaration') {
        node.left.declarations.forEach(function (d) {
          collectPattern(d.id);
        });
      }
    }
...
function executeInSandbox(instrumentedCode, stdin, limits) {
  const maxSteps =
    limits && limits.maxTraceSteps ? limits.maxTraceSteps : 10000;
  const safeL0 =
    limits && limits.safeSerializeListLimitRoot
      ? limits.safeSerializeListLimitRoot
      : 30;
  const safeLN =
    limits && limits.safeSerializeListLimitNested
      ? limits.safeSerializeListLimitNested
      : 12;
  ...
  function fakeRequire(mod) {
    ...
    throw new Error(
      "Cannot require '" +
        mod +
        "' in Frogger sandbox. Use readline() for stdin.",
    );
  }
...
function ensureAcorn() {
  ...
  try {
    importScripts(
      'https://cdn.jsdelivr.net/npm/acorn@8.14.1/dist/acorn.min.js',
    );
    acornReady = true;
